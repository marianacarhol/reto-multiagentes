...
async function rbAddHistory(h) { const { error } = await supabase.from('ticket_history_rb').insert({ ...h, ts: nowISO() }); if (error)
    throw error; }
async function mCreateTicket(row) { const { error } = await supabase.from('tickets_m').insert(row); if (error)
    throw error; }
...
async function mGetTicket(id) { const { data, error } = await supabase.from('tickets_m').select('*').eq('id', id).maybeSingle(); if (error)
    throw error; return data; }
async function mAddHistory(h) { const { error } = await supabase.from('ticket_history_m').insert({ ...h, ts: nowISO() }); if (error)
    throw error; }
// Feedback
...
async function main() {
    try {
        ...
        const jsonPath = path_1.default.resolve('./input.json');
        if (!fs_1.default.existsSync(jsonPath)) {
        ...
        const requestId = createJson?.output_data?.request_id;
        if (!requestId)
            return console.error('❌ No se obtuvo request_id');
